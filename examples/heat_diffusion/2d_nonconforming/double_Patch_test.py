...
########################################################################################################################
if left_m == "q":
    xs1 = np.linspace(0.0, 1.0, N_elem1+1)
    ...
    fens1, fes1 = t3_ablock(1, 1, N_elem1, N_elem1)
...
if right_m == "q":
    xs2 = np.linspace(1.0, 2.0, N_elem2+1)
    ...
    fens2, fes2 = t3_ablock(1, 1, N_elem2, N_elem2)
    fens2.xyz[:, 0] += 1.0
...
if top_m == "q":
    xs3 = np.linspace(0.0, 2.0, N_elem3+1)
    ys3 = np.linspace(1.0, 2.0, int(N_elem3/2)+1)
...
femm_top = FEMMHeatDiff(fes = boundary_fes3, material=m, integration_rule=GaussRule(dim=1, order=2))
F3 += femm_top.distrib_loads(geom3, T3, fi_2, 3)
F3 += femm_top.distrib_loads(geom3, T3, fi_1, 3)
