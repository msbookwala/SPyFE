...
box_b1 = np.array([0,1,0.5,0.5])
box_b2 = np.array([0,1,0.5,0.5])
########################################################################################################################
# subdomain 1
########################################################################################################################
if left_m == "q":
...
interface_fe_idx1 = np.sort(np.unique(np.hstack([interface_fe_idx1, jinterface_fe_idx1])))
...
interface_fe_idx2 = np.sort(np.unique(np.hstack([interface_fe_idx2, jinterface_fe_idx2])))
...
if top_m == "q":
    ...
    fens3, fes3 = t3_ablock(1, 0.5, N_elem3, N_elem3)
    fens3.xyz[:, 1] += 0.5
...
# interface
########################################################################################################################
ys_i1 = np.linspace(0.0, 0.5, N_elem_i+1)  # y-coordinates
xs_i1 = np.full_like(ys_i1, 0.5)     # x-coordinates (constant)
...
fens_i1.xyz[:, 0] +=  fens_i1.xyz[:, 0]*(fens_i1.xyz[:, 1]-0.25) * skew
...
xs_i2 = np.linspace(0.0, 1, int(N_elem_j)+1)  # y-coordinates
ys_i2 = np.full_like(xs_i2, 0.5)     # x-coordinates (constant)
fens_i2, fes_i2 = l2_blockx_2D(xs_i2, ys_i2)
fens_i2.xyz[:, 0] +=  fens_i2.xyz[:, 0]*(fens_i2.xyz[:, 1]-0.25) * skew

from spyfe.meshing.modification import merge_meshes
fens_i, fes_i, fes_ib = merge_meshes (fens_i1, fes_i1, fens_i2, fes_i2, tolerance=1e-9)
fes_i.conn = np.vstack([fes_i.conn , fes_i2.conn])
fes_i.label = np.vstack([fes_i.label, fes_i2.label])


C1 = build_interface_interpolator2(fens_i, fens1.xyz, boundary_fes1.conn, interface_fe_idx1, elem_lagrange)

########################################################################################################################
# Interface3
########################################################################################################################
# ys_i = np.unique(np.hstack([fens2.xyz[:, 1],fens1.xyz[:, 1]]))
xs_k = np.linspace(0.5, 1, int(N_elem_j/2)+1)  # y-coordinates
ys_k = np.full_like(xs_k, 0.5)     # x-coordinates (constant)
fens_k, fes_k = l2_blockx_2D(xs_k, ys_k)
fens_k.xyz[:, 0] +=  (1-fens_k.xyz[:, 0])*(fens_k.xyz[:, 1]-0.25) * skew
...
dbc_lam_f2 = -(C13_p @ T1_p) -(C31_p @ T3_p)


C23_p = C23[:, dbc_nodes2]
C32_p = C32[:, dbc_nodes3]
dbc_lam_f3 = - (C23_p @ T2_p)-(C32_p @ T3_p)
...
C32 = csr_matrix(np.delete(C32.toarray(), dbc_nodes3, axis = 1))
...
F = np.concatenate([F1, F2,F3, dbc_lam_f, dbc_lam_f2, dbc_lam_f3])
